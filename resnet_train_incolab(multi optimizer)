...
optimizer_dict = {
    'SGD': torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0001),
    'Adam': torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=0.0001),
    'AdaGrad': torch.optim.Adagrad(model.parameters(), lr=0.001, weight_decay=0.0001),
    'RMSprop': torch.optim.RMSprop(model.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0001),
    'AdamW': torch.optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.0001)
}
# scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_dict['SGD'], mode='min', factor=0.1, verbose=True)
# scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_dict['Adam'], mode='min', factor=0.1, verbose=True)
# scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer_dict['AdaGrad'], mode='min', factor=0.1, verbose=True)
